...
export function AccountsTable({ accounts, loading }: AccountsTableProps) {
  ...
  const SortIcon = ({ active, order }: { active: boolean; order: 'asc' | 'desc' }) => (
    <svg
      className={`w-3.5 h-3.5 ml-1 inline-block transition-colors ${active ? 'text-[#2869b0]' : 'text-[var(--muted-text)]'}`}
      fill="none"
      stroke="currentColor"
      viewBox="0 0 24 24"
    >
          ...
          <div className="flex gap-1.5 p-1 bg-[var(--background)] rounded-lg border border-[var(--border)]">
            {(['all', 'healthy', 'warning', 'critical'] as FilterType[]).map((f) => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all duration-200 ${filter === f
                  ? f === 'all'
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-[var(--border)] bg-[var(--background)]">
              <th
                className="px-5 py-3 text-left text-xs font-medium text-[var(--muted-text)] uppercase tracking-wider cursor-pointer hover:text-[var(--foreground)] transition-colors"
                onClick={() => handleSort('email')}
              >
                Email
                <SortIcon active={sortKey === 'email'} order={sortKey === 'email' ? sortOrder : 'asc'} />
              </th>
              <th className="px-5 py-3 text-left text-xs font-medium text-[var(--muted-text)] uppercase tracking-wider">
                Name
              </th>
              <th
                className="px-5 py-3 text-left text-xs font-medium text-[var(--muted-text)] uppercase tracking-wider cursor-pointer hover:text-[var(--foreground)] transition-colors"
          ...
          <tbody className="divide-y divide-[#262626]">
            {filteredAccounts.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-5 py-12 text-center text-[var(--muted-text)]">
              ...
              filteredAccounts.map((account) => (
                ...
                >
                  <td className="px-5 py-4">
                    <span className="text-[var(--foreground)] text-sm font-medium">{account.email}</span>
                  </td>
                  <td className="px-5 py-4 text-[var(--muted-text)] text-sm">
